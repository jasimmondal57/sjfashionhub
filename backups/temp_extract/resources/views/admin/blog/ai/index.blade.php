                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" id="products-grid">
                    @foreach($products as $product)
                        <div class="product-card border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow"
                            ...
                            <!-- Debug: {{ $product->main_image ? 'HAS IMAGE: ' . $product->main_image : 'NO IMAGE' }} -->
                            @if($product->main_image)
                                <img src="{{ asset($product->main_image) }}" alt="{{ $product->name }}"
                                     class="w-full h-48 object-cover"
                                     onerror="console.error('Image failed to load:', this.src); this.onerror=null; this.parentElement.innerHTML='<div class=\'w-full h-48 bg-red-100 flex items-center justify-center text-xs text-red-600\'>Image Error: {{ $product->main_image }}</div>';">
                            @else
