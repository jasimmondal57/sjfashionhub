<x-layouts.admin>
    <x-slot name="title">Create Feature - SJ Fashion Hub Admin</x-slot>
    <x-slot name="description">Create a new feature highlight</x-slot>
    <x-slot name="pageTitle">⭐ Create New Feature</x-slot>

    <!-- Header Actions -->
    <div class="flex justify-between items-center mb-6">
        <div>
            <p class="text-gray-600">Create a new feature highlight for your homepage</p>
        </div>
        ...
        <form action="{{ route('admin.features.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" 
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent"
                          placeholder="On orders above ₹999" required>{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="mb-4">
                    ...
                    <select id="icon_type" name="icon_type" 
                        ...
                        <option value="svg" {{ old('icon_type') === 'svg' ? 'selected' : '' }}>SVG Code</option>
                        <option value="image" {{ old('icon_type') === 'image' ? 'selected' : '' }}>Upload Image</option>
                        <option value="icon_class" {{ old('icon_type') === 'icon_class' ? 'selected' : '' }}>Icon Class (Font Icons)</option>
                    </select>
                    @error('icon_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div id="svg_option" class="mb-4">
                    ...
                    <textarea id="icon_svg" name="icon_svg" rows="4" 
                              ...
                              placeholder='<svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">...</svg>'>{{ old('icon_svg') }}</textarea>
                ...
                <div id="image_option" class="mb-4 hidden">
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs text-gray-500">PNG, JPG, SVG up to 1MB (32x32px recommended)</p>
                        </div>
                    </div>
                    @error('icon_image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="color" id="background_color" name="background_color" value="{{ old('background_color', '#000000') }}" 
                    ...
                    <div>
                        ...
                        <input type="color" id="icon_color" name="icon_color" value="{{ old('icon_color', '#ffffff') }}" 
                               class="w-full h-10 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent">
                        @error('icon_color')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="bg-gray-50 p-6 rounded-lg">
                    <div class="text-center max-w-xs mx-auto">
                        <div id="preview-icon" class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style="background-color: #000000;">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
                        <h3 id="preview-title" class="font-semibold text-lg mb-2">Feature Title</h3>
                        <p id="preview-description" class="text-gray-600 text-sm">Feature description</p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function updatePreview() {
            ...
            document.getElementById('preview-icon').style.backgroundColor = backgroundColor;
            
            // Update icon color
            const iconElement = document.querySelector('#preview-icon svg');
            if (iconElement) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('icon_color').addEventListener('change', updatePreview);
        });
    </script>
</x-layouts.admin>
