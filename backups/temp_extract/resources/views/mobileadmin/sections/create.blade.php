<x-layouts.mobileadmin>
    <div class="p-6">
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-1">Add a new section to the app home screen</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="type" 
                            ...
                            <option value="banner" {{ old('type') == 'banner' ? 'selected' : '' }}>Banner Slider</option>
                            <option value="category" {{ old('type') == 'category' ? 'selected' : '' }}>Category Grid</option>
                            <option value="product_grid" {{ old('type') == 'product_grid' ? 'selected' : '' }}>Product Grid</option>
                            <option value="product_carousel" {{ old('type') == 'product_carousel' ? 'selected' : '' }}>Product Carousel</option>
                            <option value="featured" {{ old('type') == 'featured' ? 'selected' : '' }}>Featured Products</option>
                            <option value="deals" {{ old('type') == 'deals' ? 'selected' : '' }}>Deals & Offers</option>
                            <option value="custom" {{ old('type') == 'custom' ? 'selected' : '' }}>Custom Section</option>
                        </select>
                        @error('type')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="config" 
                                  ...
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 font-mono text-sm">{{ old('config') }}</textarea>
                        @error('config')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
