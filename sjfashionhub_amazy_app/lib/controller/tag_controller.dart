...
class TagController extends GetxController {
  ...
  Future<TagProductsModel> getTagProducts(tagId) async {
    ...
    try {
      isTagLoading(true);
      await _dio.get(URLs.SINGLE_TAG_PRODUCTS + '/$tagId',queryParameters: { "lang" : AppLocalizations.getLanguageCode()}).then((value) {
        ...
        log("tagAllData.value  :::: ${tagAllData.value.categoryList?.length}");
        ...
        if (tagAllData.value.products?.data?.length != 0) {
          ...
          if ((tagAllData.value.attributeLists?.length??0) > 0) {
