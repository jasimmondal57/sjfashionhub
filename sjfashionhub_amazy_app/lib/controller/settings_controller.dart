...
import 'package:sjfashionhub/model/NewModel/Currency.dart';
import 'package:sjfashionhub/model/NewModel/GeneralSettingsModel.dart';
import 'package:sjfashionhub/model/NewModel/Product/GiftCardData.dart';
...
import 'package:sjfashionhub/view/amazcart_view/SplashScreen.dart';
...
class GeneralSettingsController extends GetxController {
  ...
  var appCurrency = ''.obs;
  var currencySymbolPosition = ''.obs;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (settingsModel.value.msg?.toLowerCase() == 'success') {
          ...
          languages.value = settingsModel.value.languages??[];

          log("languages.value ::: ${languages.length}");
          ...
          currencySymbolPosition.value = settingsModel.value.settings!.currencyPosition!;
          ...
          if (settingsModel.value.otpConfiguration != null) {
            ...
            if (settingsModel.value.otpConfiguration?.firstWhere((element) => element.type == 'otp_on_customer_registration').value == 1) {
              otpOnCustomerRegistration.value = true;
            } else {
              otpOnCustomerRegistration.value = false;
            }

            if (settingsModel.value.otpConfiguration?.firstWhere((element) => element.type == 'otp_on_login').value == 1) {
  ...
  String calculateMainPrice(ProductModel productModel) {
    ...
    if (currencySymbolPosition == 'left_with_space'){
      amountText = "${appCurrency.value} $amountText";
    }else{
      amountText = "$amountText${appCurrency.value}";
    }
  ...
  String calculatePrice(ProductModel prod) {
    if (prod.productType == ProductType.GIFT_CARD) {
      if (prod.giftCardEndDate!.compareTo(DateTime.now()) > 0) {
        priceText = singlePrice(sellingPrice(prod.giftCardSellingPrice, prod.discountType, prod.discount)).toStringAsFixed(2);
      } else {
      ...
      if (prod.hasDeal != null) {
        if (prod.product!.productType == 1) {
          priceText = singlePrice(sellingPrice(prod.skus!.first.sellingPrice, prod.hasDeal!.discountType, prod.hasDeal!.discount)).toStringAsFixed(2);
        }
        else {
        ...
        if (prod.product?.productType == 1)
        {
          ...
          if (sellingPrice(prod.minSellPrice??0, prod.discountType, prod.discount??0) == sellingPrice(prod.maxSellingPrice??0, prod.discountType, prod.discount??0)) {
            if (prod.hasDiscount == 'yes') {
              priceText = singlePrice(sellingPrice(
                      prod.skus?.first.sellingPrice??0,
                      prod.discountType,
                      prod.discount))
                  .toStringAsFixed(2);
            } else {
              log("Product name ::: ${prod.productName} ::: ${prod.minSellPrice}");

              priceText = singlePrice(prod.minSellPrice??0)
                  .toStringAsFixed(2);
            }
          } else {
            var priceA;
            ...
            if (prod.hasDiscount == 'yes') {
              ...
              priceA = singlePrice(prod.minSellPrice??0).toStringAsFixed(2);
    ...
    return setCurrentSymbolPosition(amount: priceText??'0');
  }
  ...
  String calculateMainPriceWithVariant(ProductModel productModel) {
    ...
    if (productModel.hasDiscount == 'yes' || productModel.hasDeal != null) {
      if (productModel.product!.productType == 1) {
        ...
        amountText = setCurrentSymbolPosition(amount: amountText);
      } else {
        amountText = setCurrentSymbolPosition(amount: double.parse(((productModel.minSellPrice??0) * conversionRate.value).toString()).toStringAsFixed(2))
            +
            ' - '
            +
            setCurrentSymbolPosition(amount: double.parse((productModel.maxSellingPrice! * conversionRate.value)
                .toString())
                .toStringAsFixed(2));
      }
    } else {
      ...
      if (prod.product?.productType == 1) {
        ...
        if (sellingPrice(prod.minSellPrice, prod.discountType, prod.discount) ==
          ...
          if (prod.hasDiscount == 'yes') {
            ...
            priceText =
                singlePrice(prod.skus?.first.sellingPrice??0).toStringAsFixed(2);
          }
        } else {
          var priceA;
          var priceB;
  ...
  dynamic sellingPrice(amount, discountType, discountAmount) {
    ...
    var sellingPrice = (amount??0) - (discount??0);
    return sellingPrice;
  }
...
String setCurrentSymbolPosition({required String amount}){

  if(currencySymbolPosition == 'left_with_space') {
    return "${appCurrency.value}$amount";
  }else{
    return "$amount${appCurrency.value}";
  }
