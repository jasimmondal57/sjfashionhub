...
import 'package:sjfashionhub/model/MyWishListModel.dart';
...
class MyWishListController extends GetxController {
  ...
  Future<MyWishListModel> getWishlists() async {
    ...
    Uri userData = Uri.parse(
      URLs.MY_WISHLIST + '?lang=${AppLocalizations.getLanguageCode()}',
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // Use adapter to convert response
        var adaptedResponse = ApiAdapter.adaptWishlist(jsonString);
        return MyWishListModel.fromJson(adaptedResponse);
      } else {
        print('Wishlist API error: ${response.statusCode} - ${response.body}');
        return MyWishListModel();
      }
    } catch (e) {
      print('Wishlist error: $e');
      // Return empty wishlist model on error
      var emptyResponse = ApiAdapter.adaptWishlist(null);
      return MyWishListModel.fromJson(emptyResponse);
    }
  }
        ...
        wishListModel.value.products?.values.forEach((element) {
          ...
          element.forEach((element2) {
            if (element2.type == ProductType.GIFT_CARD) {
              if (element2.giftcard?.id != null) {
                wishListProducts.add(
                  WishListLocal(
                    productId: element2.sellerProductId ?? 0,
                    id: element2.id,
  ...
  Future deleteWishListProduct(id) async {
    ...
    Uri userData = Uri.parse(
      URLs.MY_WISHLIST_DELETE + '?lang=${AppLocalizations.getLanguageCode()}',
    );
    print('delete product ::: ${URLs.MY_WISHLIST_DELETE}');
    print('delete token ::: $token');

    Map data = {"id": id, "type": "product", "seller_id": 1};
