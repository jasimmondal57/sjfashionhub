...
import 'package:sjfashionhub/model/NewModel/AllRecommendedModel.dart';
import 'package:sjfashionhub/model/NewModel/FilterFromCatModel.dart';
import 'package:sjfashionhub/model/NewModel/FlashDeals/FlashDealData.dart';
import 'package:sjfashionhub/model/NewModel/FlashDeals/FlashDealDataElement.dart';
import 'package:sjfashionhub/model/NewModel/FlashDeals/FlashDealModel.dart';
import 'package:sjfashionhub/model/NewModel/Brand/BrandData.dart';
import 'package:sjfashionhub/model/NewModel/Brand/SingleBrandModel.dart';
import 'package:sjfashionhub/model/NewModel/Category/CategoryData.dart';
import 'package:sjfashionhub/model/NewModel/Category/ParentCategory.dart';
import 'package:sjfashionhub/model/NewModel/Category/SingleCategory.dart';
import 'package:sjfashionhub/model/NewModel/Filter/FilterAttributeValue.dart';
import 'package:sjfashionhub/model/NewModel/Filter/FilterColor.dart';
import 'package:sjfashionhub/model/NewModel/HomePage/HomePageModel.dart';
import 'package:sjfashionhub/model/NewModel/Product/AllProducts.dart';
import 'package:sjfashionhub/model/NewModel/Product/ProductModel.dart';
import 'package:sjfashionhub/model/NewUserZoneModel.dart' as NewUser;
import 'package:sjfashionhub/model/SliderModel.dart';
...
import 'package:sjfashionhub/model/NewModel/Category/CategoryMain.dart';
import 'package:sjfashionhub/model/NewModel/Brand/BrandsMain.dart';

import '../view/amazcart_view/products/RecommendedProductLoadMore.dart';

final Rx<GlobalKey<ScaffoldState>> scaffoldkey = GlobalKey<ScaffoldState>().obs;
...
class HomeController extends GetxController {
  ...
  Future<void> getAllCategory() async {
    try {
      ...
      await _dio
          ...
          .then((value) {
            CategoryMain dat = CategoryMain.fromJson(
    ...
    try {
      ...
      await _dio
          ...
          .then((value) async {
            ...
            chunkedBrands.clear();
            chunkedBrands.addAll(
              allBrands.where((p0) => p0.featured == 1).toList(),
            );
          });
    } catch (e) {
      ...
      await http
          ...
          .then((value) async {
            if (value is FlashDealModel) {
              flashDealData = value.flashDeal!.allProducts!.data!
                  .take(10)
                  .toList();
              ...
              } else if (value.flashDeal!.endDate!.millisecondsSinceEpoch <
                  now) {
      ...
      await _dio
          ...
          .then((value) async {
            if (value is NewUser.NewUserZoneModel) {
              newUserZoneProducts.value = value.newUserZone!.allProducts!.data!
                  .take(2)
                  .toList();
  ...
  addIfNoAttribute({
    ...
    dataFilterCat.value.filterDataFromCat?.filterType?.add(
      FilterType(filterTypeId: typeId.toString(), filterTypeValue: []),
    );
  }
    ...
    try {
      ...
      await _dio
          ...
          .then((value) {
            ...
            if ((catAllData.value.attributes?.length ?? 0) > 0) {
              catAllData.value.attributes?.forEach((element) {
                dataFilterCat.value.filterDataFromCat?.filterType?.add(
                  FilterType(
                    filterTypeId: element.id.toString(),
                    filterTypeValue: [],
                  ),
                );
              });
            ...
            if ((catAllData.value.brands?.length ?? 0) > 0) {
              dataFilterCat.value.filterDataFromCat?.filterType?.add(
                FilterType(filterTypeId: 'brand', filterTypeValue: []),
              );
            }
            if ((catAllData.value.data?.subCategories?.length ?? 0) > 0) {
              dataFilterCat.value.filterDataFromCat?.filterType?.add(
                FilterType(filterTypeId: 'cat', filterTypeValue: []),
              );
            }
            ...
            dataFilterCat.value.filterDataFromCat?.filterType?.add(
              FilterType(filterTypeId: 'rating', filterTypeValue: []),
            );
            ...
            categoryImage.value =
                parentCategoryElement.categoryImage?.image ?? "";
      ...
      await _dio
          ...
          .then((value) {
            ...
            if (brandAllData.value.attributes != null &&
                (brandAllData.value.attributes?.length ?? 0) > 0) {
            ...
            if (brandAllData.value.categories != null &&
                (brandAllData.value.categories?.length ?? 0) > 0) {
              dataFilterCat.value.filterDataFromCat?.filterType?.add(
                FilterType(filterTypeId: 'cat', filterTypeValue: []),
              );
            }
            ...
            brandId.value = allBrandModelDatum.id ?? 0;
  ...
  Future<List<ProductModel>> filterBrandProducts() async {
    ...
    dataFilterCat.value.filterDataFromCat?.filterType?.removeWhere(
      (element) =>
  ...
  Future<List<FlashDealDataElement>> getFlashDealsData() async {
    ...
    try {
      ...
      await _dio
          .get(
            URLs.FLASH_DEALS + '?lang=${AppLocalizations.getLanguageCode()}',
            queryParameters: {'page': flashPageNumber},
          )
          .then((value) async {
            ...
            if (flashDeals.startDate!.millisecondsSinceEpoch > now) {
              dealDuration.value = Duration(
                milliseconds:
                    flashDeals.startDate!.millisecondsSinceEpoch - now,
              );
  ...
  Future getWishList() async {
    ...
    final MyWishListController _myWishListController = Get.put(
  ...
  Future getHomePage() async {
    ...
    try {
      ...
      print("🚀 Home Controller: Starting HTTP call to ${URLs.HOME_PAGE}");

      Response response = await _dio
          .get(URLs.HOME_PAGE)
          .timeout(Duration(seconds: 10));
      print(
        "✅ Home Controller: HTTP call completed with status: ${response.statusCode}",
      );
      print(
        "📊 Home Controller: Response data type: ${response.data.runtimeType}",
      );
      print(
        "📝 Home Controller: Response data: ${response.data.toString().substring(0, 200)}...",
      );

      Map<String, dynamic> data = Map<String, dynamic>.from(response.data);

      // Use adapter to convert SJFashionHub response to Amazy format
      print("🔄 Home Controller: About to call ApiAdapter.adaptHome");
      var adaptedData = ApiAdapter.adaptHome(data);
      print("🎯 Home Controller: Adapted data keys: ${adaptedData.keys}");
      print(
        "📂 Home Controller: Categories count: ${adaptedData['top_categories']?.length}",
      );
      print(
        "🏷️ Home Controller: Featured brands count: ${adaptedData['featured_brands']?.length}",
      );
      print(
        "⭐ Home Controller: Top picks count: ${adaptedData['top_picks']?.length}",
      );

      HomePageModel model = HomePageModel.fromJson(adaptedData);
      print("Home Controller: Model created successfully");
      print(
        "Home Controller: Model categories: ${model.topCategories?.length}",
      );
      print(
        "Home Controller: Model featured brands: ${model.featuredBrands?.length}",
      );
      print("Home Controller: Model top picks: ${model.topPicks?.length}");

      model.newUserZone?.allProducts?.removeWhere(
        (element) => element.product == null,
      );
      model.flashDeal?.allProducts?.removeWhere(
        (element) => element.product == null,
      );
      homePageModel.value = model;
      print("Home Controller: HomePageModel set to controller");
      print(
      ...
      chunkedBrands.clear();
      if (homePageModel.value.featuredBrands != null) {
        chunkedBrands.addAll(homePageModel.value.featuredBrands!.toList());
        print("Home Controller: Added ${chunkedBrands.length} featured brands");
      } else {
        print("Home Controller: No featured brands available");
      }
    } catch (e, t) {
      isHomePageLoading(false);
      print("❌ Home Controller Catch Error: ${e.toString()}");
      print("🔍 Home Controller Catch Track: ${t.toString()}");
      if (e.toString().contains('TimeoutException')) {
        print("⏰ HTTP call timed out after 10 seconds");
      }
    } finally {
      isHomePageLoading(false);
      print("🏁 Home Controller: Finally block executed");
    }
  }
