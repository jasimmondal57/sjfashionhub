...
import 'package:sjfashionhub/view/amazcart_view/support/CreateTicketPage.dart';
    ...
    try {
      ...
      if (this.length == 0) {
        result = await _dio.get(
          URLs.TICKET_LIST + '?lang=${AppLocalizations.getLanguageCode()}',
          options: Options(
            headers: {
        ...
        result = await _dio.get(URLs.TICKET_LIST + '?lang=${AppLocalizations.getLanguageCode()}',
            options: Options(
              headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
