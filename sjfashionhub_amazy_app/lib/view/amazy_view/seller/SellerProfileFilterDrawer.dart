...
class _SellerProfileFilterDrawerState extends State<SellerProfileFilterDrawer> {
  ...
  void initState() {
    ...
    controller = Get.put(SellerProfileController());
                              ...
                              child: MaterialButton(
                                onPressed: () async {
                                  ...
                                  if (controller!.selectedSubCat
                                   ...
                                   // controller!.listCategories.add(item.value.id.toString());
                ...
                ? Builder(
                    builder: (context) {
                      ...
                      return MultiSelectChipField<BrandData?>(
                        ...
                        itemBuilder: (MultiSelectItem<BrandData?> item, FormFieldState<List<BrandData?>> state) {
                          return Card(
                            color: controller!.selectedBrand.contains(item.value)
                                ? AppStyles.darkBlueColor
                                : Colors.white,
                            elevation: controller!.selectedBrand.contains(item.value) ? 5 : 3,
                            shape: RoundedRectangleBorder(
                            ...
                            child: Container(
                              ...
                              child: MaterialButton(
                                onPressed: () async {
                                  ...
                                  if (controller!.selectedBrand.contains(item.value)) {
                                    controller!.selectedBrand.remove(item.value);
                                    //controller!.listBrands.remove(item.value!.id.toString());
                                    await doFilter();
                                  } else {
                                    controller!.selectedBrand.add(item.value ?? BrandData());
                                   // controller!.listBrands.add(item.value!.id.toString());
                                    await doFilter();
                                  }
                                },
                                child: Text(
                                  item.value!.name!,
                                  style: controller!.selectedBrand.contains(item.value)
                                      ? AppStyles.kFontWhite14w5
                                      : AppStyles.kFontBlack14w5,
                                ),
                              ),
                ...
                ? Column(
                    ...
                    children: [
                      ...
                      Padding(
                        padding: EdgeInsets.symmetric(horizontal: 15),
                        child: Text(
                          '${"Price Range".tr} (${_currencyController.appCurrency.value})',
                          style: AppStyles.kFontBlack12w4,
                        ),
                      ),
                    ],
