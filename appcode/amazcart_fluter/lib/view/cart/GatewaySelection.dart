...
// import 'package:pay/pay.dart';  // Temporarily disabled due to Kotlin compatibility issues
...
class _GatewaySelectionState extends State<GatewaySelection> {
  final PaymentGatewayController controller = Get.put(
  ...
  // final _paymentItems = <PaymentItem>[]; // Temporarily disabled due to pay package removal
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBarWidget(title: 'Select Gateway'.tr),
      body: Obx(() {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    width: 0.2,
                    color:
                        controller.gatewayList[position] ==
                ...
                child: RadioListTile<Gateway>(
                  ...
                  title: Row(
                    ...
                    children: [
                      Text(
                        "${controller.gatewayList[position].method ?? ''}".tr,
            ...
            children: [
              ...
              Obx(() {
                if (controller.isPaymentProcessing.value) {
                  ...
                  // Google Pay and Apple Pay temporarily disabled due to Kotlin compatibility issues
                  return selectedGooglePay
                      ? Container(
                          ...
                          child: Center(
                            child: Text(
                              'Google Pay/Apple Pay temporarily unavailable',
  ...
  Future orderConfirmSubmit() async {
    ...
    if (controller.selectedGateway.value.id == 1) {
      ...
      log(
        "_checkoutController.orderData['grand_total'] ::: ${_checkoutController.orderData}",
      );
      Map payment = {
    ...
    /// Wallet
      ...
      await _accountController.getAccountDetails().then((value) async {
        if (double.parse(
          ...
          SnackBars().snackBarWarning(
            "You don't have sufficient wallet balance".tr,
          );
        } else {
          _checkoutController.orderData.addAll({
            'wallet_amount':
      ...
      Get.to(
        () => PaypalPayment(
          onFinish: (number) async {
            payment.addAll({'transection_id': number});
            await controller.paymentInfoStore(
              paymentData: payment,
    ...
    ///Stripe
      ...
      int amount =
          (double.parse(
                    _checkoutController.orderData['grand_total'] != null
                        ? _checkoutController.orderData['grand_total']
                              .toString()
                        : "0.0",
                  ) *
                  100)
      ...
      if (transactionId.isNotEmpty) {
        payment.addAll({'transection_id': transactionId});
        await controller.paymentInfoStore(
          paymentData: payment,
          transactionID: transactionId,
        );
      } else {
        showAlertDialog(context, "Error".tr, "Something went wrong".tr);
      }
    }
    ///PayStack
      ...
      final request = PaystackTransactionRequest(
        ...
        email: _checkoutController.orderData['customer_email'],
        amount:
            ((num.tryParse("${_checkoutController.orderData['grand_total']}") ??
                        0) *
                    100)
                .toDouble(),
      ...
      await PaymentService.showPaymentModal(
        ...
        callbackUrl:
            '${AppConfig.hostUrl}/payment_gateway_success_callback/Paystack',
      );
    ...
    ///Jazzcash
      ...
      Get.bottomSheet(
        JazzCashSheet(orderData: _checkoutController.orderData),
        isScrollControlled: true,
        backgroundColor: Colors.transparent,
        persistent: true,
      );
