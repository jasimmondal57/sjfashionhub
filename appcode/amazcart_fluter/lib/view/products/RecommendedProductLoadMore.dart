...
class RecommendedProductsLoadMore extends LoadingMoreBase<ProductModel> {
  ...
  Future<bool> loadData([bool isloadMoreAction = false]) async {
    ...
    try {
      ...
      print("AllRecommendedModel created, meta: ${source.meta}");
      // Handle total as either int or string
      var total = source.meta?.total;
      print("total value: $total, type: ${total.runtimeType}");
      if (total is int) {
        productsLength = total;
      } else if (total is String) {
        productsLength = int.tryParse(total) ?? 0;
      } else {
        productsLength = 0;
      }
      print("productsLength set to: $productsLength");
