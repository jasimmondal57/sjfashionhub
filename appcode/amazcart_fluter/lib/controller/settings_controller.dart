  ...
  Future<GeneralSettingsModel> getGeneralSettings() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        settingsModel.value = GeneralSettingsModel.fromJson(
        ...
        if (settingsModel.value.msg?.toLowerCase() == 'success') {
          ...
          currencySymbolPosition.value =
              settingsModel.value.settings!.currencyPosition!;
          ...
          if (settingsModel.value.otpConfiguration != null) {
            ...
            otpCodeValidationTime.value = int.parse(
              settingsModel.value.otpConfiguration!
                  .firstWhere(
  ...
  String calculateMainPrice(ProductModel productModel) {
    ...
    if (productModel.productType == ProductType.GIFT_CARD) {
      amountText = double.parse(
        (productModel.giftCardSellingPrice! * conversionRate.value).toString(),
      ).toStringAsFixed(2);
    } else {
      if (productModel.hasDiscount == 'yes' || productModel.hasDeal != null) {
        if (productModel.product!.productType == 1) {
          amountText = double.parse(
            (productModel.maxSellingPrice! * conversionRate.value).toString(),
          ).toStringAsFixed(2);
        } else {
          amountText = double.parse(
            (productModel.maxSellingPrice! * conversionRate.value).toString(),
    ...
    if (prod.productType == ProductType.GIFT_CARD) {
      ...
      if (prod.hasDeal != null) {
        if (prod.product!.productType == 1) {
          priceText = singlePrice(
            sellingPrice(
              prod.skus?.isNotEmpty == true
                  ? prod.skus!.first.sellingPrice
                  : prod.maxSellingPrice ?? 0,
              prod.hasDeal!.discountType,
        ...
        if (prod.product?.productType == 1) {
          ...
          if (sellingPrice(
            ...
            if (prod.hasDiscount == 'yes') {
              ...
              log(
                "Product name ::: ${prod.productName} ::: ${prod.minSellPrice}",
              );
      ...
      if (productModel.product!.productType == 1) {
        ...
        amountText =
            setCurrentSymbolPosition(
              amount: double.parse(
                ((productModel.minSellPrice ?? 0) * conversionRate.value)
                    .toString(),
              ).toStringAsFixed(2),
            ) +
            ' - ' +
