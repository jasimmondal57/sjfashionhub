...
class ProductModel {
  ...
  factory ProductModel.fromJson(Map<String, dynamic> json) {
    // Check if this is our simple format (has 'name' and 'price') or complex format (has 'product_name' and 'min_sell_price')
    bool isSimpleFormat = json.containsKey('name') && json.containsKey('price');

    if (isSimpleFormat) {
      // Handle our simple site format
      double price = AppUtilities.convertToDouble(item: json["price"]) ?? 0.0;
      double? salePrice = json["sale_price"] != null
          ? AppUtilities.convertToDouble(item: json["sale_price"])
          : null;
      double sellingPrice = salePrice ?? price;
      ...
      return ProductModel(
        ...
        tax: 0,
        taxType: "percent",
        discount: salePrice != null ? ((price - salePrice) / price * 100) : 0,
        ...
        slug: json["slug"],
        thumImg: json["image"],
        status: json["in_stock"] == true ? 1 : 0,
        stockManage: 1,
        isApproved: 1,
        minSellPrice: sellingPrice,
        maxSellPrice: sellingPrice,
        totalSale: 0,
        avgRating: AppUtilities.convertToDouble(item: json["rating"]) ?? 0.0,
        maxSellingPrice: sellingPrice,
        hasDeal: null,
        rating: AppUtilities.convertToDouble(item: json["rating"]) ?? 0.0,
        hasDiscount: salePrice != null ? 'yes' : 'no',
        product: null,
        seller: null,
        reviews: [],
        skus: [],
      ...
      // Handle complex format (original code)
      return ProductModel(
        id: AppUtilities.convertToInt(item: json["id"]),
        ...
        seller: json["seller"] != null && json["seller"].isNotEmpty
