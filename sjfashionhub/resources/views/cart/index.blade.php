                            ...
                            <div class="p-6">
                                <div class="space-y-6">
                                    @foreach($cartItems as $item)
                                        <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm" data-item-id="{{ $item->id }}">
                                            <div class="flex flex-col lg:flex-row lg:items-center lg:space-x-6 space-y-4 lg:space-y-0">
                                                <!-- Product Image -->
                                                <div class="flex-shrink-0">
                                                    @if($item->product->featured_image)
                                                        <img class="h-24 w-24 rounded-lg object-cover border border-gray-200" src="{{ Storage::url($item->product->featured_image) }}" alt="{{ $item->product->name }}">
                                                    @else
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <p class="text-sm text-gray-600 mb-3">{{ $item->product->category->name ?? 'No Category' }}</p>
                                                    @php
                                                ...
                                                <div class="flex flex-col lg:flex-row lg:items-center lg:space-x-6 space-y-4 lg:space-y-0">
                                                    ...
                                                    <div class="flex items-center">
                                                        ...
                                                        <div class="flex items-center border border-gray-300 rounded-lg bg-white">
                                                            <button class="px-3 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-l-lg" onclick="updateQuantity({{ $item->id }}, -1)">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"></path>
                                                                </svg>
                                                            </button>
                                                            <span class="px-4 py-2 text-gray-900 font-semibold bg-gray-50 quantity-display min-w-[3rem] text-center">{{ $item->quantity }}</span>
                                                            <button class="px-3 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-r-lg" onclick="updateQuantity({{ $item->id }}, 1)">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <!-- Remove Button -->
                                                    <button class="text-red-600 hover:text-red-500 hover:bg-red-50 p-2 rounded-lg transition-colors" onclick="removeItem({{ $item->id }})" title="Remove item">
